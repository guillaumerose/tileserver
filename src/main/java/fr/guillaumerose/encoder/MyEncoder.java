package fr.guillaumerose.encoder;
...
public class MyEncoder {
    ...
    /**
     * Create a {@link VectorTileEncoder} with the default extent of 4096 and clip buffer of 8.
     */
    public MyEncoder() {
        this(4096, 8, true);
    }
    ...
    /**
     * Create a {@link VectorTileEncoder} with the given extent and a clip buffer of 8.
     */
    public MyEncoder(int extent) {
        this(extent, 8, true);
    }
     ...
     * <p>
     * The extent value control how detailed the coordinates are encoded in the vector tile. 4096 is
     * a good default, 256 can be used to reduce density.
     * <p>
     * The clip buffer value control how large the clipping area is outside of the tile for
     * geometries. 0 means that the clipping is done at the tile border. 8 is a good default.
     *
     * @param extent
     ...
     * @param autoScale
     *            when true, the encoder expects coordinates in the 0..255 range and will scale them
     *            automatically to the 0..extent-1 range before encoding. when false, the encoder
     *            expects coordinates in the 0..extent-1 range.
     */
    public MyEncoder(int extent, int clipBuffer, boolean autoScale) {
        this.extent = extent;
     ...
     * <p>
     * For optimization, geometries will be clipped, geometries will simplified and features with
     ...
     * performance. This method can be overridden to change clipping behavior. See also
     * {@link #clipGeometry(Geometry)}.
     * 
    ...
    List<Integer> commands(Geometry geometry) {
        ...
        if (geometry instanceof Polygon) {
            ...
            // According to the vector tile specification, the exterior ring of
            // a polygon
            // must be in clockwise order, while the interior ring in
            ...
            // So, the code below will make sure that exterior ring is in
            // counter-clockwise order
